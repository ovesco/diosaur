...
import { BaseInjectedService, ConstructorInjectedService, AttributeInjectedService } from "../Metadata/ServiceInjection";
import { BaseInjectedParameter, ConstructorInjectedParameter, AttributeInjectedParameter } from "../Metadata/ParameterInjection";
import { BaseInjectAllService, ConstructorInjectAllService, AttributeInjectAllService } from "../Metadata/AllServiceInjection";
...
class ServiceResolver {
    ...
    private identifierTags: Map<ServiceIdentifier, Array<string | null>> = new Map();

    private scopeServices: Map<string, Map<ServiceIdentifier, Map<string | null, Object>>> = new Map();

    private singletons: Map<ServiceIdentifier, Map<string | null, Object>> = new Map();
    ...
    constructor(private factories: RegisteredFactory[],
        ...
        private parameters: ParameterBag) {
            ...
            factories.forEach((injection) => {
                ...
                if (!this.identifierTags.has(identifier)) this.identifierTags.set(identifier, []);
                if (this.identifierTags.get(identifier)?.includes(tag)) {
                    throw new Error(`Trying to register 2 services identified with ${identifier.toString()} and tag ${tag?.toString()}`);
                }
    ...
    public async warmup() {
        ...
        for (const singleton of singletons) {
            ...
            if (!this.singletons.has(config.identifier)) this.singletons.set(config.identifier, new Map());
    ...
    public exitScope(scope: string) {
        if (this.scopeServices.has(scope)) {
            ...
            const services: Map<ServiceIdentifier, Map<string | null, Object>> = this.scopeServices.get(scope) as Map<ServiceIdentifier, Map<string | null, Object>>;
            this.scopeServices.delete(scope);
            services.forEach((tagsServices, identifier) => {
                tagsServices.forEach((service, tag) => {
                    const { config } = this.graph.getNode(DependencyGraph.serviceKey(identifier, tag))?.data as RegisteredFactory;
                    ...
                    if (intersectingScopes.length > 0) {
                        ...
                        if (!this.scopeServices.get(nextScope)?.has(identifier)) this.scopeServices.get(nextScope)?.set(identifier, new Map());
                        this.scopeServices.get(nextScope)?.get(identifier)?.set(tag, service);
                    }
                });
            });
        }
        ...
        switch (serviceConfig.config.scoping) {
            ...
            case SCOPES.custom:
                ...
                for (const scope of availableScopes) {
                    const service = this.scopeServices.get(scope)?.get(identifier)?.get(tag);
                ...
                if (availableScopes.length > 0) {
                    ...
                    if (!this.scopeServices.has(scope)) this.scopeServices.set(scope, new Map());
                    if (!this.scopeServices.get(scope)?.has(identifier)) this.scopeServices.get(scope)?.set(identifier, new Map());
    ...
    private postServiceCreation(service: object, attributes: { key: string | symbol, arg: any }[]) {
    ...
    private setTo(
        ...
        attributes: { key: string | symbol, arg: any }[], arg: any,
    ...
    private buildServiceDependencies(node: Node): [any[], { key: string | symbol, arg: any }[]] {
    ...
    private async buildServiceDependenciesAsync(node: Node): Promise<[any[], { key: string | symbol, arg: any }[]]> {
        const parameters: { arg: any, index: number }[] = [];
        const attributes: { key: string | symbol, arg: any }[] = [];
